...
def createPart(model, sketch, partName):
    ...
    myPart = model.Part(name=partName, dimensionality=THREE_D, type=DEFORMABLE_BODY)
...
def createSection(model, part, materialName, sectionName):
    ...
    mySection = model.HomogeneousSolidSection(name=sectionName, material=materialName, thickness=1.0)
...
def createEncastreBoundaryCondition(instance, model, part, createStepName, boundaryConditionName):
    ...
    model.EncastreBC(name=boundaryConditionName, createStepName=createStepName, region=region)
...
encastreBoundaryCondition = createEncastreBoundaryCondition(instance, model, part, INITIAL_STEP_NAME, BOUNDARY_CONDITION_NAME)
pressure = createPressure(model, instance, LOAD_STEP_NAME, PRESSURE_NAME)

optimizations.optimize_topology(session, part, MODEL_NAME, LOAD_STEP_NAME, 1.25, 2.5)
